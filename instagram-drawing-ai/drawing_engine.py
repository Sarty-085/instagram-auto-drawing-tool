...
def execute_drawing(
    ...
    for layer_num, layer in enumerate(layers_data, start=1):
        ...
        if mode in ("hatch", "directional"):
            # --- Directional hatching fill (gradient-aware polylines) -------
            from smart_strokes import get_directional_hatching_paths
            fill_step: int = int(drawing_cfg["fill_step_size"])
            # Load original image if available in layer dict or use dummy
            img_bgr = layer.get("img_bgr", np.zeros((h_mask, w_mask, 3), dtype=np.uint8))
            paths = get_directional_hatching_paths(mask, img_bgr, step_size=fill_step)
            for path in paths:
                if len(path) < 2:
                    continue
                ...
                for i in range(len(pts) - 1):
                    ...
                    sx, sy = int(pt1[0]) + x_phone, int(pt1[1]) + y_phone
                    ex, ey = int(pt2[0]) + x_phone, int(pt2[1]) + y_phone
                    sx, sy, ex, ey = make_swipe_coordinates_safe(sx, sy, ex, ey, safe_x)
                    dur = get_swipe_duration(sx, sy, ex, ey, config)
                    adb.swipe(sx, sy, ex, ey, dur)
                    time.sleep(inter_delay)
                time.sleep(max(0.1, inter_delay))
